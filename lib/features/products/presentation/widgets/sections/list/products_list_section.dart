...
/// Product list with infinite scroll pagination and search support
class SectionProductList extends ConsumerStatefulWidget {
  ...
  final ValueChanged<String> onProductEdit;
  final ValueChanged<String> onProductDelete;
  final ScrollController? scrollController;

  @override
  ConsumerState<SectionProductList> createState() =>
      _SectionProductListState();
}

class _SectionProductListState extends ConsumerState<SectionProductList> {
  static final _log = AppLogger.getLogger(name: 'SectionProductList');

  late final PagingController<int, ProductData> _pagingController;
  var _hasInitialPageLoaded = false;

  @override
  void initState() {
    super.initState();
    _pagingController = PagingController<int, ProductData>(
  ...
  @override
  void didUpdateWidget(covariant final SectionProductList oldWidget) {
  ...
  Future<List<ProductData>> _fetchPage(final int pageKey) async {
    try {
      final allProducts =
          await ref.read(productsActionsProvider).getAllActiveProducts();
  ...
  List<ProductData> _filterAndSortProducts(final List<ProductData> products) {
  ...
  Future<void> _handleRefresh() async {
    try {
      await ref.read(productsActionsProvider).syncWithSupabase();
    } catch (e, s) {
  ...
  Widget build(final BuildContext context) {
    ref.listen<AsyncValue<List<ProductData>>>(productsStreamProvider, (
      final prev,
      final next,
    ) {
    ...
    return RefreshIndicator(
      ...
      child: PagingListener<int, ProductData>(
        ...
        builder: (final context, final state, final fetchNextPage) {
          ...
          return PagedListView<int, ProductData>.separated(
              state: state,
              ...
              builderDelegate: PagedChildBuilderDelegate<ProductData>(
                ...
                itemBuilder:
                    (final context, final product, final index) =>
                        ProductListItem(
                          product: product,
                          onTap: () =>
                              widget.onProductTap(product.id),
                          onEdit: () =>
                              widget.onProductEdit(product.id),
                          onDelete: () =>
                              widget.onProductDelete(product.id),
                          index: index,
                        ),
